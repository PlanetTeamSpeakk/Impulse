...
import com.impulsebot.miscellaneous.SilentCommandEvent;
...
public class Moderation {
	...
	static {
		...
		Main.addCommandHook(event -> {
			...
			if (event.getGuild() != null && blacklist.containsKey(event.getGuild().getId()) && ((List) blacklist.get(event.getGuild().getId())).contains(event.getAuthor().getId()))
				throw new CommandPermissionException("You're blacklisted in this server.");
		...
		Main.addCommandHook(event -> {
			if (event.getGuild() != null) if (event.getCommand().isAnnotationPresent(Command.class)) {
				Command command = event.getCommand().getAnnotation(Command.class);
				if (!Dashboard.getEnabledModules(event.getGuild()).contains(command.category().toLowerCase()) && command.obeyDashboard()) throw new CommandPermissionException("That command cannot be used as its category, %s, isn't enabled.", command.category());
				...
				if (!Dashboard.getEnabledModules(event.getGuild()).contains(Main.getAbsoluteParentCommand(command).getAnnotation(Command.class).category().toLowerCase()) && command.obeyDashboard()) throw new CommandPermissionException("That command cannot be used as its category, %s, isn't enabled.", Main.getAbsoluteParentCommand(command).getAnnotation(Command.class).category());
			}
		});
	...
	public static void ban(CommandEvent event) {
		if (!event.getArgs().isEmpty()) {
			...
			else if (!PermissionUtil.canInteract(event.getGuild().getSelfMember(), member))
				event.reply("I cannot ban that user as they're higher in the hierarchy than I am.");
			else {
	...
	public static void kick(CommandEvent event) {
		if (!event.getArgs().isEmpty()) {
			...
			if (member == null)
				...
				event.reply("I cannot kick that user as they're higher in the hierarchy than I am.");
			else {
				event.getGuild().getController().kick(member).complete();
				event.reply("Successfully kicked " + member.getEffectiveName() + ".");
				logModAction(member.getUser(), event.getAuthor(), event.getGuild(), null, "Kick", "boot");
			}
		} else Main.sendCommandHelp(event);
	}
	...
	public static void mute(CommandEvent event) {
		if (!event.getArgs().isEmpty()) {
			...
			if (member == null)
				...
				event.reply("I cannot mute that user as they're higher in the hierarchy than I am.");
			else {
				Main.mute(member);
				event.reply("Successfully muted " + member.getEffectiveName() + ".");
				logModAction(member.getUser(), event.getAuthor(), event.getGuild(), null, "Mute", "mute");
			}
		} else Main.sendCommandHelp(event);
	}
	...
	public static void unmute(CommandEvent event) throws CommandException {
		if (!event.getArgs().isEmpty()) {
			...
			if (member == null)
				event.reply("The given user could not be found.");
			else if (!settings.containsKey(event.getGuild().getId()) || !((List) ((Map) settings.get(event.getGuild().getId())).get("mutes")).contains(member.getUser().getId()))
				event.reply("That user has not been muted using this bot, make sure to always set the mod channel first with %smodset channel.", Main.getPrefix(event.getGuild()));
			else if (!PermissionUtil.canInteract(event.getGuild().getSelfMember(), member))
				event.reply("I cannot unmute that user as they're higher in the hierarchy than I am.");
			else {
				((List) ((Map) settings.get(event.getGuild().getId())).get("mutes")).remove(member.getUser().getId());
				try {
					DataIO.saveJson(settings, "data/mod/settings.json");
				} catch (IOException e) {
					throw new CommandException("An unknown error occurred while saving the settings file.", e);
				}
				Main.unmute(member);
				event.reply("Successfully unmuted " + member.getEffectiveName() + ".");
				logModAction(member.getUser(), event.getAuthor(), event.getGuild(), null, "Unmute", "speaker");
			}
		} else Main.sendCommandHelp(event);
	}

	private static void logModAction(User user, User moderator, Guild guild, String reason, String actionType, String emoji) {
		if (settings.containsKey(guild.getId())) {
			if (actionType.toLowerCase().equals("mute")) ((List) ((Map) settings.get(guild.getId())).get("mutes")).add(user.getId());
			...
			if (channel != null) {
				...
				String messageId = channel.sendMessageFormat("**Case #%s** | %s :%s:\n" + "**User:** %s (%s)\n" + "**Moderator:** %s\n" + "**Reason:** %s", caseNum, actionType, emoji, Main.str(user), user.getId(), moderator == null ? "Unknown" : moderator.getName() + "#" + moderator.getDiscriminator() + " (" + moderator.getId() + ")", Main.getPrefix(guild), reason == null ? "Unknown, type %sreason " + caseNum + " <reason> to add it." : reason).complete().getId();
				((Map) ((Map) settings.get(guild.getId())).get("cases")).put("" + caseNum, Main.newHashMap(new String[] {"messageId", "user", "moderator", "reason", "caseNum"}, new Object[] {messageId, user.getId(), moderator == null ? "" : moderator.getId(), reason, caseNum}));
				try {
					DataIO.saveJson(settings, "data/mod/settings.json");
				} catch (IOException e) {
			...
			if (settings.containsKey(event.getGuild().getId())) {
				...
				if (!cases.isEmpty() && cases.size() >= Integer.parseInt(event.getArgs().split(" ")[0])) {
					...
					if (channel != null) {
						...
						if (message != null) {
							if (!message.getAuthor().getId().equals(event.getSelfMember().getUser().getId()))
								event.reply("I cannot edit the message attached to that case.");
							else {
								message.editMessage(message.getContentRaw().substring(0, message.getContentRaw().lastIndexOf("**") + 3) + Main.join(Main.removeArg(event.getArgs().split(" "), 0))).queue();
	...
	@Subcommand(help = "Add a user to this server's blacklist.", name = "add", arguments = "<user>", parent = "com.impulsebot.commands.Moderation.blacklist", guildOnly = true)
	...
	@Subcommand(help = "Removes a user from the blacklist.", name = "remove", arguments = "<user>", parent = "com.impulsebot.commands.Moderation.blacklist", guildOnly = true)
	public static void blacklistRemove(CommandEvent event) throws CommandException {
		if (!event.getArgs().isEmpty()) {
			Member member = Main.getMemberFromInput(event.getMessage());
			if (member == null)
				event.reply("That user could not be found.");
			else if (!blacklist.containsKey(event.getGuild().getId()))
				event.reply("No one in this server is currently blacklisted.");
			else if (!((List) blacklist.get(event.getGuild().getId())).contains(member.getUser().getId()))
	...
	@Subcommand(help = "Manage the global blacklist.", name = "global", parent = "com.impulsebot.commands.Moderation.blacklist", ownerCommand = true)
	public static void blacklistGlobal(CommandEvent event) {
		Main.sendCommandHelp(event);
	}

	@Subcommand(help = "Add a user to the global blacklist.", name = "add", arguments = "<user>", parent = "com.impulsebot.commands.Moderation.blacklistGlobal", ownerCommand = true)
	...
	@Subcommand(help = "Removes a user from the global blacklist.", name = "remove", arguments = "<user>", parent = "com.impulsebot.commands.Moderation.blacklistGlobal", ownerCommand = true)
	public static void blacklistGlobalRemove(CommandEvent event) throws CommandException {
		if (!event.getArgs().isEmpty()) {
			Member member = Main.getMemberFromInput(event.getMessage());
			if (member == null)
				event.reply("That user could not be found.");
			else if (!blacklist.containsKey("global"))
				event.reply("No one is currently globally blacklisted.");
			else if (!((List) blacklist.get("global")).contains(member.getUser().getId()))
				event.reply("That user isn't blacklisted.");
			else {
	...
	@Subcommand(help = "Add a filter to the list of filters.", name = "add", parent = "com.impulsebot.commands.Moderation.filter", arguments = "<filter>", userPermissions = {Permission.MESSAGE_MANAGE}, guildOnly = true)
	...
	@Subcommand(help = "Remove a filter from the list of filters.", name = "remove", parent = "com.impulsebot.commands.Moderation.filter", arguments = "<filter>", userPermissions = {Permission.MESSAGE_MANAGE}, guildOnly = true)
	public static void filterRemove(CommandEvent event) throws CommandException {
		if (!event.getArgs().isEmpty()) {
			if (!filters.containsKey(event.getGuild().getId()))
				event.reply("Nothing is being filtered in this server.");
			else if (!((List) ((Map) filters.get(event.getGuild().getId())).get("filtered")).contains(event.getArgs()))
				event.reply("The given message is currently not being filtered.");
			else {
	...
	@Subcommand(help = "Lists all filters in this server.", name = "list", parent = "com.impulsebot.commands.Moderation.filter", guildOnly = true)
	...
	public static void giveme(CommandEvent event) {
		if (!event.getArgs().isEmpty()) {
			if (!givemeSettings.containsKey(event.getGuild().getId()))
				event.reply("This server has no givemes yet.");
			else {
				String givemeName = "";
				...
				if (!((Map) givemeSettings.get(event.getGuild().getId())).containsKey(givemeName))
					event.reply("That giveme could not be found.");
				else {
					Role giveme = event.getGuild().getRoleById((String) ((Map) givemeSettings.get(event.getGuild().getId())).get(givemeName));
					if (giveme == null)
						event.reply("The givemes was found, but the connected role was deleted.");
					else if (!PermissionUtil.canInteract(event.getSelfMember(), giveme))
						event.reply("I cannot give you that role, as it is higher in the hierarchy than my highest role.");
					else {
	...
	@Subcommand(help = "Add givemes, divide the roles with a semi-colon (;).", name = "add", parent = "com.impulsebot.commands.Moderation.giveme", guildOnly = true, userPermissions = {Permission.MANAGE_ROLES}, arguments = "<roles>")
		...
		if (!event.argsEmpty()) {
			...
			for (String roleName : event.getArgs().split(event.getArgs().contains("; ") ? "; " : ";"))
				if (event.getGuild().getRolesByName(roleName, true).size() == 0) {
					event.reply("The role '%s' could not be found.", roleName);
					return;
			...
			event.reply("Successfully added %s giveme%s.", roles.size(), roles.size() == 1 ? "" : "s");
		} else Main.sendCommandHelp(event);
	}

	@Subcommand(help = "Lists all givemes in this server.", name = "list", parent = "com.impulsebot.commands.Moderation.giveme", guildOnly = true)
	public static void givemeList(CommandEvent event) {
		if (!givemeSettings.containsKey(event.getGuild().getId())) givemeSettings.put(event.getGuild().getId(), new HashMap());
		if (((Map) givemeSettings.get(event.getGuild().getId())).keySet().isEmpty())
			event.reply("This server has no givemes.");
	...
	@Subcommand(help = "Remove a giveme.", name = "remove", parent = "com.impulsebot.commands.Moderation.giveme", guildOnly = true, arguments = "<giveme>")
		...
		if (!event.argsEmpty()) {
			if (!givemeSettings.containsKey(event.getGuild().getId()))
				...
				for (String giveme : event.getArgs().split(";"))
					if (!((Map) givemeSettings.get(event.getGuild().getId())).containsKey(giveme)) {
						event.reply("The giveme '%s' could not be found.", giveme);
						return;
					} else {
				...
				event.reply("Successfully removed %s giveme%s.", counter, counter == 1 ? "" : "s");
			}
		} else Main.sendCommandHelp(event);
	}

	@Subcommand(help = "Remove a giveme from your roles.", name = "getoff", parent = "com.impulsebot.commands.Moderation.giveme", arguments = "<giveme>", botPermissions = {Permission.MANAGE_ROLES})
		...
		if (!event.getArgs().isEmpty()) {
			if (!givemeSettings.containsKey(event.getGuild().getId()))
				...
				if (!((Map) givemeSettings.get(event.getGuild().getId())).containsKey(givemeName))
					...
					if (giveme == null)
						event.reply("The giveme was found, but the connected role was deleted.");
					else if (!PermissionUtil.canInteract(event.getSelfMember(), giveme))
						event.reply("I cannot remove that role from you, as it is higher in the hierarchy than my highest role.");
					else if (!event.getMember().getRoles().contains(giveme))
						event.reply("You do not have that giveme on you.");
					else {
	...
	public static void leave(CommandEvent event) {
		...
		else if (response.getContentDisplay().toLowerCase().startsWith("ye")) {
	...
	public static void modset(CommandEvent event) {
		...
		if (settings.containsKey(event.getGuild().getId())) {
			...
			String welcomeChannelId = (String) ((Map) settings.get(event.getGuild().getId())).get("welcomeChannel");
			...
			useImage = ((Map) settings.get(event.getGuild().getId())).containsKey("image") ? ((Map) settings.get(event.getGuild().getId())).get("image").toString() : useImage;
		}
	...
	@Subcommand(help = "Sets this server's prefix.", name = "serverprefix", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Set the channel to which kicks, bans and mutes are logged to.", name = "channel", parent = "com.impulsebot.commands.Moderation.modset", arguments = "<channel>", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Sets if the bot should ban users who sent a message which mentions more than 8 users.", name = "banmentionspam", parent = "com.impulsebot.commands.Moderation.modset", arguments = "<flag>", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Set the role members should automatically get assigned when they join.", name = "autorole", parent = "com.impulsebot.commands.Moderation.modset", guildOnly = true, userPermissions = {Permission.MANAGE_ROLES}, arguments = "<role>")
	...
	@Subcommand(help = "Toggle whether new members should automatically get a role assigned when they join.", name = "toggleautorole", parent = "com.impulsebot.commands.Moderation.modset", guildOnly = true, userPermissions = {Permission.MANAGE_ROLES})
	...
	@Subcommand(help = "Set the message that should be sent when an old user leaves.\nYou can use variables in this such as:\nUSER this'll be the user's name and discriminator.\nUSER_MENTION this'll mention the user.\nSERVER this'll be the server's name.", name = "farewell", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Sets whether greetings and farewells should be sent in DMs.", name = "dm", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Sets whether greetings and farewells should be sent in image form.", name = "useimage", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Sets the channel in which the bot should greet and farewell users.", name = "welcomechannel", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Disable the welcome system.", name = "disable", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	public static void modsetDisableGreeting(CommandEvent event) throws CommandException {
		if (!settings.containsKey(event.getGuild().getId()))
			...
			((Map) settings.get(event.getGuild().getId())).put("welcomeChannel", "");
			((Map) settings.get(event.getGuild().getId())).put("useImage", false);
		}
		try {
			DataIO.saveJson(settings, "data/mod/settings.json");
		} catch (IOException e) {
	...
	@Subcommand(help = "Toggles the invite removing system.", name = "toggleinviteremoving", parent = "com.impulsebot.commands.Moderation.modset", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	public static void modsetToggleInviteRemoving(CommandEvent event) throws CommandException {
		...
		event.reply("Successfully toggled the invite removing system to %s.", ((Map) settings.get(event.getGuild().getId())).get("inviteRemoving"));
	}
	...
	public static void pastNames(CommandEvent event) {
		if (!event.argsEmpty()) {
			...
			if (user == null)
				event.reply("That user could not be found.");
			else if (!pastNames.containsKey(user.getId()))
				event.reply("No data could be found for that user.");
	...
	public static void pastNicks(CommandEvent event) {
		if (!event.argsEmpty()) {
			...
			if (user == null)
				event.reply("That user could not be found.");
			else if (!pastNicks.containsKey(user.getId()))
				event.reply("No data could be found for that user.");
	...
	public static void rename(CommandEvent event) {
		if (!event.getArgs().isEmpty()) {
			Member member = Main.getMemberFromInput(event.getMessage());
			if (member == null)
				event.reply("That member could not be found.");
			else if (!PermissionUtil.canInteract(event.getSelfMember(), member))
				event.reply("I cannot rename that user as they're higher in the hierarchy than I am.");
			else {
				String newName = "";
				if (event.getMessage().getMentionedUsers().isEmpty())
					newName = Main.join(Main.removeArgs(event.getArgs().split(" "), Main.range(member.getUser().getName().split(" ").length)));
				else newName = Main.join(Main.removeArg(event.getArgs().split(" "), 0));
	...
	public static void resetWarns(CommandEvent event) {
		if (!event.argsEmpty()) {
			...
			if (!warnerSettings.containsKey(guildId))
				event.reply("No one in this server currently has any warnings.");
			else if (!((Map) warnerSettings.get(guildId)).containsKey(userId))
				event.reply("That user has no warnings.");
			else {
	...
	public static void warn(CommandEvent event) {
		if (!event.argsEmpty()) {
			...
			if (!event.getGuild().getSelfMember().canInteract(member))
				...
				switch (warnings) {
				case 0: {
					output = "%s has **0 warnings**, dafuq? :thinking:";
					break;
				}
				case 1: {
					output = "%s has **1 warning**, but nothing happens yet. Next up: **5 minute mute**.";
					break;
				}
				case 2: {
					output = "%s has **2 warnings** and has been muted for 5 minutes. Next up: **30 minute mute**.";
					mute(member, 5);
					break;
				}
				case 3: {
					output = "%s has **3 warnings** and has been muted for 30 minutes. Next up: **kick**.";
					mute(member, 30);
					break;
				}
				case 4: {
					output = "%s has **4 warnings** and has been kicked. Next up: **ban**.";
					event.getGuild().getController().kick(member).complete();
					break;
				}
				case 5: {
					output = "%s has **5 warnings** and has been banned.";
					resetWarns(new SilentCommandEvent(event));
					event.getGuild().getController().ban(member, 1).complete();
					break;
				}
				default: {
					output = "%s has **an unknown** amount of warnings. :thinking:";
					break;
				}
				}
				try {
	...
	@Subcommand(help = "Set the channel modlog should log to.", name = "channel", parent = "com.impulsebot.commands.Moderation.modlogset", userPermissions = {Permission.ADMINISTRATOR}, guildOnly = true, arguments = "<channel>")
	...
	@Subcommand(help = "Toggle whether the modlog is enabled.\nThe enabled arg should be either true or false.", name = "toggle", parent = "com.impulsebot.commands.Moderation.modlogset", userPermissions = {Permission.ADMINISTRATOR}, guildOnly = true, arguments = "<enabled>")
	...
	@Subcommand(help = "Create a key to use on <https://dashboard.impulsebot.com>.", name = "getkey", parent = "com.impulsebot.commands.Moderation.dashboard", guildOnly = true, userPermissions = {Permission.ADMINISTRATOR}, obeyDashboard = false)
	...
	public static void purge(CommandEvent event) {
		if (!event.argsEmpty() && Main.isInteger(event.getArgs()) && Integer.parseInt(event.getArgs()) < 100 && Integer.parseInt(event.getArgs()) > 0) {
			...
			event.getTextChannel().getHistory().retrievePast(Integer.parseInt(event.getArgs())).complete().forEach(m -> {
				if (m.getCreationTime().toEpochSecond() > minEpoch) ids.add(m.getId());
			});
			if (ids.size() > 2) {
				event.getTextChannel().deleteMessagesByIds(ids).queue();
				event.reply("Successfully deleted %s messages.", ids.size() - 1);
			} else event.reply("Could not find 2 or more messages of at most 2 weeks old.");
		} else Main.sendCommandHelp(event);
	}

	@Command(category = "Moderation", help = "Moves all users connected to the 'from' channel to the 'to' channel.\n\nSplit with a semi-colon. (;)", name = "massmove", arguments = "<from>;<to>", userPermissions = {Permission.VOICE_MOVE_OTHERS}, botPermissions = {Permission.VOICE_MOVE_OTHERS})
	public static void massmove(CommandEvent event) {
		if (!event.argsEmpty() && event.getArgs().split(";").length >= 2) {
			...
			if (from == null || to == null)
				event.reply("Could not find a voice channel with the name '%s'.", event.getArgs().split(";")[from == null ? 0 : 1]);
			else {
	...
	private static void log(Guild guild, String emote, String name, String content, Object... formatArgs) {
		if (modlogSettings.containsKey(guild.getId()) && (boolean) modlogSettings.get(guild.getId()).get("enabled") && !modlogSettings.get(guild.getId()).get("channel").toString().isEmpty()) {
			...
			if (channel != null) channel.sendMessageFormat("`[%s]` :%s: **%s Log**\n```java\n%s```", Main.getFormattedTime(), emote, name, String.format(content, formatArgs)).queue();
		}
	}

	@SubscribeEvent
	public static void onGuildMemberJoin(GuildMemberJoinEvent event) {
		if (settings.containsKey(event.getGuild().getId()) && ((Map) settings.get(event.getGuild().getId())).containsKey("mutes") && ((List) ((Map) settings.get(event.getGuild().getId())).get("mutes")).contains(event.getUser().getId())) Main.mute(event.getGuild().getMember(event.getUser()));
		if (settings.containsKey(event.getGuild().getId()) && !((Map) settings.get(event.getGuild().getId())).get("autorole").toString().isEmpty() && event.getGuild().getRoleById(((Map) settings.get(event.getGuild().getId())).get("autorole").toString()) != null) try {
		...
		if (settings.containsKey(event.getGuild().getId()) && !((Map) settings.get(event.getGuild().getId())).get("welcomeChannel").toString().isEmpty() && event.getGuild().getTextChannelById(((Map) settings.get(event.getGuild().getId())).get("welcomeChannel").toString()) != null) {
			...
			if (((Map) settings.get(event.getGuild().getId())).containsKey("image") && (boolean) ((Map) settings.get(event.getGuild().getId())).get("image")) {
				...
				try {
					BufferedImage imageOut = ImageManipulator.toBufferedImage(ImageManipulator.generateWelcomeImage(event.getMember()), true);
			...
			if (cont) channel.sendMessage(((Map) settings.get(event.getGuild().getId())).get("greeting").toString().replaceAll("USER_MENTION", event.getUser().getAsMention()).replaceAll("USER", Main.str(event.getUser())).replaceAll("SERVER", Main.str(event.getGuild()))).queue();
		}
	...
	public static void onGuildMemberLeave(GuildMemberLeaveEvent event) {
		if (settings.containsKey(event.getGuild().getId()) && !((Map) settings.get(event.getGuild().getId())).get("welcomeChannel").toString().isEmpty() && event.getGuild().getTextChannelById(((Map) settings.get(event.getGuild().getId())).get("welcomeChannel").toString()) != null) event.getGuild().getTextChannelById(((Map) settings.get(event.getGuild().getId())).get("welcomeChannel").toString()).sendMessage(((Map) settings.get(event.getGuild().getId())).get("farewell").toString().replaceAll("USER_MENTION", event.getUser().getAsMention()).replaceAll("USER", Main.str(event.getUser())).replaceAll("SERVER", Main.str(event.getGuild()))).queue();
	...
	public static void onGuildBan(GuildBanEvent event) {
		User moderator = null;
		String reason = null;
		if (Main.getSendChannel(event.getGuild()) != null && Main.hasPermissions(Main.getSendChannel(event.getGuild()), Main.getSelfUser(), Permission.VIEW_AUDIT_LOGS)) {
			List<AuditLogEntry> entries = event.getGuild().getAuditLogs().type(ActionType.BAN).complete();
			for (AuditLogEntry entry : entries)
				if (entry.getTargetId().equals(event.getUser().getId())) {
					moderator = entry.getUser(); // getting whoever banned this person using the audit logs.
					reason = entry.getReason();
					break;
				}
		}
		logModAction(event.getUser(), moderator, event.getGuild(), reason, "Ban", "hammer");
	...
	public static void onMessageUpdate(MessageUpdateEvent event) {
		log(event.getGuild(), "pencil2", "Message Edit", "Member: %s\nChannel: %s\nBefore: %s\nAfter: %s", Main.str(event.getAuthor()), event.getChannel().getName(), loggedMessages.containsKey(event.getMessageId()) ? loggedMessages.get(event.getMessageId()).get("content") : "Unknown", event.getMessage().getContentDisplay());
		loggedMessages.put(event.getMessage().getId(), Main.newHashMap(new String[] {"content", "sent", "author"}, new Object[] {event.getMessage().getContentRaw(), System.currentTimeMillis(), event.getAuthor().getId()})); // ik it's updating the sent variable, it is supposed to.
	}

	@SubscribeEvent
	public static void onMessageDelete(MessageDeleteEvent event) {
		if (loggedMessages.containsKey(event.getMessageId()) && !Main.getSelfUser().getId().equals(loggedMessages.get(event.getMessageId()).get("author"))) log(event.getGuild(), "wastebasket", "Message Delete", "Member: %s\nChannel: %s\nMessage: %s", !loggedMessages.containsKey(event.getMessageId()) ? "Unknown" : Main.str(Main.getUserById(loggedMessages.get(event.getMessageId()).get("author").toString())), event.getTextChannel().getName(), !loggedMessages.containsKey(event.getMessageId()) ? "Unknown" : loggedMessages.get(event.getMessageId()).get("content").toString());
	}
	...
	public static void onGenericRoleUpdate(GenericRoleUpdateEvent event) {
		...
		log(event.getGuild(), "game_die", "Role Update", "Before:" + "\n\tName:            %s" + "\n\tColour:          %s" + "\n\tMentionable:     %s" + "\n\tHoisted:         %s" + "\n\tPermissions:\n\t\t%s" + "\n\nAfter:" + "\n\tName:            %s" + "\n\tColour:          %s" + "\n\tMentionable:     %s" + "\n\tHoisted:         %s" + "\n\tPermissions:\n\t\t%s", before.getName(), Main.colourToHex(before.getColor()), before.isMentionable(), before.isHoisted(), Main.joinCustomChar("\n\t\t", permsBefore), after.getName(), Main.colourToHex(after.getColor()), after.isMentionable(), after.isHoisted(), Main.joinCustomChar("\n\t\t", permsAfter));
		roles.put(event.getRole().getId(), Main.cloneRole(event.getRole()));
	}
	...
	private static void mute(Member member, int minutes) {
		...
		for (Channel channel : Main.getAllChannels(member.getGuild())) {
			if (channel.getPermissionOverride(member) != null)
				perms.put(channel, channel.getPermissionOverride(member).getAllowedRaw());
		...
		Main.runAsynchronously(() -> {
			...
			for (Channel channel : perms.keySet())
				if (channel.getGuild().getTextChannelById(channel.getId()) != null || channel.getGuild().getVoiceChannelById(channel.getId()) != null) if (perms.get(channel) != null)
					channel.getPermissionOverride(member).getManager().grant(perms.get(channel)).queue();
	...
	public static void onMessageReceived(MessageReceivedEvent event) {
		if (event.getMessage().getContentRaw().equals(Main.getSelfUser().getAsMention())) event.getChannel().sendMessageFormat("My prefix here is `%s`.", Main.getPrefix(event.getGuild())).queue();
		loggedMessages.put(event.getMessage().getId(), Main.newHashMap(new String[] {"content", "sent", "author"}, new Object[] {event.getMessage().getContentRaw(), System.currentTimeMillis(), event.getAuthor().getId()}));
		...
		if (filters.containsKey(event.getGuild().getId()) && !event.getAuthor().getId().equals(Main.getSelfUser().getId()) && !event.getMember().hasPermission(Permission.MESSAGE_MANAGE)) {
			...
			if (Main.contains(event.getMessage().getContentRaw().toLowerCase(), filters)) {
				try {
	...
	public static void onGuildMemberNickChange(GuildMemberNickChangeEvent event) {
		if (!pastNicks.containsKey(event.getUser().getId()))
			pastNicks.put(event.getUser().getId(), Lists.newArrayList(event.getPrevNick()));
	...
	@SubscribeEvent
	public static void onUserUpdateName(UserUpdateNameEvent event) {
		if (!pastNames.containsKey(event.getUser().getId()))
			pastNicks.put(event.getUser().getId(), Lists.newArrayList(event.getOldName()));
	...
	public static Map getSettings(Guild guild) {
		...
		} // there used to be a bug in the dashboard that didn't convert channel names to
			// their IDs.
		return settings;
	}
