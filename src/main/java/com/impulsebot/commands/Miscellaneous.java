...
import com.impulsebot.Main.TimeType;
...
import com.impulsebot.utils.MailServer;
import com.impulsebot.utils.Random;
import com.impulsebot.utils.Url2Png;
...
public class Miscellaneous {
	...
	@Subcommand(help = "Create a custom command. \nExample: [p]customcom create does this work?; Yes, it does.", name = "create", parent = "com.impulsebot.commands.Miscellaneous.customCom", arguments = "<command>; <message>", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Remove a custom command.", name = "remove", parent = "com.impulsebot.commands.Miscellaneous.customCom", arguments = "<command>", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Edit a custom command.", name = "edit", parent = "com.impulsebot.commands.Miscellaneous.customCom", arguments = "<command>; <message>", userPermissions = {Permission.KICK_MEMBERS}, guildOnly = true)
	...
	@Subcommand(help = "Creates an email account.", name = "create", parent = "com.impulsebot.commands.Miscellaneous.email", dmOnly = true, arguments = "<password>")
		...
		if (!event.argsEmpty()) {
			if (MailServer.isEnabled()) {
				if (event.getArgs().length() >= 8) {
					...
					if (success)
						event.reply("Successfully made an email account, login info:\n\tEmail address: %s@%s\n\tPassword: %s\n\nTo see how to log in on any email client type %semail options", event.getAuthor().getId(), Config.INSTANCE.get("mailBaseUrl"), event.getArgs(), Main.getPrefix(event.getGuild()));
	...
	@Subcommand(help = "Tells you the options to log in on any email client.", name = "options", parent = "com.impulsebot.commands.Miscellaneous.email")
	public static void emailOptions(CommandEvent event) {
		if (MailServer.isEnabled())
			event.reply(new EmbedBuilder().setColor(new Color(random.randInt(256 * 256 * 256))).addField("Type", Boolean.parseBoolean(Config.INSTANCE.get("miabIMAP")) ? "IMAP" : "POP", true).addField("Server", Config.INSTANCE.get("miabServer"), true).addField("Incoming port", Config.INSTANCE.get("miabIncomingPort"), true).addField("Outgoing port", Config.INSTANCE.get("miabOutgoingPort"), true).addField("Incoming security", Config.INSTANCE.get("miabIncomingSecurity"), true).addField("Outgoing security", Config.INSTANCE.get("miabOutgoingSecurity"), true).addField("SMTP always verify", Config.INSTANCE.get("miabSmtpAlwaysVerify"), true).build());
		else event.reply("This feature has not been enabled by the owner of this bot.");
	}
	...
	@Subcommand(help = "Create your custom patron email address.", name = "create", parent = "com.impulsebot.commands.Miscellaneous.pemail", arguments = "<name> <password>", dmOnly = true)
		...
		if (isPatron(event.getAuthor()) && MailServer.isEnabled()) {
			if (event.getArgs().split(" ").length >= 2) {
				...
				if (password.length() >= 8) {
					try {
						MailServer.createMailAddress(name, password, patronEmails.containsKey(event.getAuthor().getId()) ? patronEmails.get(event.getAuthor().getId()).containsKey(name + "@" + Config.INSTANCE.get("mailBaseUrl")) : false);
					} catch (IOException e) {
					...
					patronEmails.get(event.getAuthor().getId()).put(name + "@" + Config.INSTANCE.get("mailBaseUrl"), password);
					try {
					...
					event.reply("Successfully made your custom email address, your email address is **%s@%s** and your password is **%s**, type %spemail options for the options used to log in.", name, Config.INSTANCE.get("mailBaseUrl"), password, Main.getPrefix(null));
				} else event.reply("The password must be a minimum of 8 characters long.");
			} else Main.sendCommandHelp(event);
		} else event.reply("You're not a patron or this feature has not been enabled by the owner of this bot so you cannot use this command.");
	}

	@Subcommand(help = "Lists all your custom patron emails, to make it show passwords run it with [p]pemail list -showpass.", name = "list", parent = "com.impulsebot.commands.Miscellaneous.pemail")
	...
	@Subcommand(help = "Shows you all the options necessary for logging in on any email client.", name = "options", parent = "com.impulsebot.commands.Miscellaneous.pemail")
	...
	@Subcommand(help = "Open a rift.", name = "open", parent = "com.impulsebot.commands.Miscellaneous.rift", guildOnly = true, arguments = "<query> [guild]", userPermissions = {Permission.MESSAGE_MANAGE}, botPermissions = {Permission.MANAGE_WEBHOOKS})
	public static void riftOpen(CommandEvent event) {
		if (!event.argsEmpty()) {
			...
			if (!possibilities.isEmpty()) {
				...
				int count = 0;
				while ((response == null || !Main.isInteger(response.getContentDisplay()) || Integer.parseInt(response.getContentDisplay()) > possibilities.size()) && count++ < 4) {
					...
					if (response == null || response.getContentDisplay().equalsIgnoreCase("cancel")) {
				...
				TextChannel chosenOne = possibilities.get(Integer.parseInt(response.getContentDisplay()) - 1);
				...
				if (chosenOne.canTalk() && chosenOne.getGuild().getSelfMember().hasPermission(Permission.MANAGE_WEBHOOKS)) {
					...
					if (response != null && response.getContentDisplay().toLowerCase().startsWith("ye")) {
	...
	@Subcommand(help = "Close a rift.", name = "close", parent = "com.impulsebot.commands.Miscellaneous.rift", guildOnly = true, userPermissions = {Permission.MESSAGE_MANAGE})
			...
			while (true) {
				if (!akinator.getGuessesAboveProbability(probability).isEmpty()) {
					...
					if (response == null) {
						...
						return;
					} else if (response.getContentDisplay().toLowerCase().startsWith("no"))
						if (probability < 1d)
							probability += 0.05;
						else {
				...
				if (response == null || response.getContentDisplay().equalsIgnoreCase("stop")) {
					...
					return;
				} else switch (response.getContentDisplay().toLowerCase()) {
				case "y":
				case "yes":
	...
	public static void colour(CommandEvent event) throws CommandException {
		if (event.getArgs().split(" ")[0].startsWith("#") && event.getArgs().split(" ")[0].length() == 7 && Main.isHexInteger(event.getArgs().split(" ")[0].substring(1)) || event.getArgs().split(" ")[0].length() == 6 && Main.isHexInteger(event.getArgs().split(" ")[0])) {
			...
			event.getChannel().sendFile(new File("data/tmp/colour_" + Main.colourToHex(g.getColor()) + "_" + size + ".png")).complete();
	...
	public static void onMessageReceived(MessageReceivedEvent event) {
		...
		if (event.getMessage().getContentDisplay().startsWith(serverPrefix)) if (settings.containsKey(event.getGuild().getId()) && ((Map) settings.get(event.getGuild().getId())).containsKey(event.getMessage().getContentDisplay().substring(serverPrefix.length()))) {
			try {
			...
			event.getChannel().sendMessage((String) ((Map) settings.get(event.getGuild().getId())).get(event.getMessage().getContentDisplay().substring(serverPrefix.length()))).queue();
		}
		...
		if (rifts.containsKey(event.getChannel().getId()) && !event.getMessage().isWebhookMessage() && (channel = Main.getTextChannelById(rifts.get(event.getChannel().getId()))) != null) while (true) {
			...
			if (!event.getMessage().getContentRaw().isEmpty()) client.get().send(event.getMessage().getContentRaw()).whenComplete((r, t) -> {
			});
