...
import com.impulsebot.utils.Cleverbot;
import com.impulsebot.utils.Downloader;
import com.impulsebot.utils.Downloader.DownloadResult;
...
public class General {
		...
		if (event.getArgs().isEmpty()) {
			...
			for (String category : CommandManager.getCategories()) {
				...
				for (String cmdName : Main.sort(Main.getCommandNames())) {
					Command cmd = Main.getCommandByName(cmdName).toCommand();
			...
			for (CommandContainer cmdM : CommandManager.getCommands()) {
				Command cmd = cmdM.toCommand();
				String[] args = event.getArgs().split(" ");
				if (cmd.name().equals(args[0])) {
					args = Main.removeArg(args, 0);
					while (args.length > 0) {
						for (CommandContainer scmd : CommandManager.getSubcommands(cmdM))
							if (scmd.toSubcommand().name().equals(args[0])) {
								cmdM = scmd;
								break;
							}
						args = Main.removeArg(args, 0);
					}
					Main.sendCommandHelp(event, cmdM.toMethod());
					return;
				}
			}
			for (String category : CommandManager.getCategories())
	...
	public static void ping(CommandEvent event) throws ClassNotFoundException {
	...
	public static void serverCount(CommandEvent event) {
		if (Main.getShardCount() > 1)
			event.reply("This shard is currently in **%s** servers and can see **%s** users.\n" + "This bot is currently in **%s** servers and can see **%s** users.", event.getJDA().getGuilds().size(), event.getJDA().getUsers().size(), Main.getTotalGuildCount(), Main.getTotalUserCount());
	...
	@Subcommand(help = "Gives you a lot of information about a user.", name = "getuserinfo", parent = "com.impulsebot.commands.General.steam", cooldown = 30, arguments = "<user>")
	...
	@Subcommand(help = "See what game or app is attached to this mysterious id :O, or just use a name, whatever you like.", name = "applookup", parent = "com.impulsebot.commands.General.steam", cooldown = 30, arguments = "<game>")
	...
	@Subcommand(help = "Shows you the top 100 games played by players since march 2009.", name = "top100forever", parent = "com.impulsebot.commands.General.steam", cooldown = 30)
	...
	@Subcommand(help = "Shows you the top 100 games played by players since the last 2 weeks.", name = "top100in2weeks", parent = "com.impulsebot.commands.General.steam", cooldown = 30)
	...
	@Subcommand(help = "Tells you how many apps are currently on Steam.", name = "appcount", parent = "com.impulsebot.commands.General.steam")
	...
	@Subcommand(help = "Get CS:GO stats from a user.\nExample:\n[p]steam getcsgostats PlanetTeamSpeak (Warning: I am noob)\n[p]steam getcsgostats 76561198187354157 (Same user but with Steam 64 ID)", name = "getcsgostats", parent = "com.impulsebot.commands.General.steam", cooldown = 30)
	...
	@Subcommand(help = "Get some user info.\nUser has to be the Wargaming username and server has to be eu, ru, asia, na, or kr.", name = "getuserinfo", parent = "com.impulsebot.commands.General.wot", cooldown = 30, arguments = "<user> <server>", guildOnly = true)
				...
				if (data.containsKey("error"))
					...
					if (Main.getIntFromPossibleDouble(((Map) data.get("meta")).get("count")) > 1) {
						...
						if (response == null) {
							...
							return;
						} else if (Main.isInteger(response.getContentDisplay())) {
							int choice = Integer.parseInt(response.getContentDisplay());
	...
	@Subcommand(help = "Get some tank info.", name = "gettankinfo", parent = "com.impulsebot.commands.General.wot", arguments = "<tank>", cooldown = 30)
	...
	@Subcommand(help = "Shows you the server icon.", name = "icon", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	public static void serverIcon(CommandEvent event) {
		event.reply(event.getGuild().getIconUrl());
	}

	@Subcommand(help = "Shows you all the custom emotes this server has.", name = "emotes", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	...
	@Subcommand(help = "Shows you all the roles this server has.", name = "roles", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	...
	@Subcommand(help = "Tells you who the owner of this server is.", name = "owner", parent = "com.impulsebot.commands.General.server", guildOnly = true, cooldown = 60)
	...
	@Subcommand(help = "Shows you some information about the given role.", name = "roleinfo", parent = "com.impulsebot.commands.General.server", guildOnly = true, arguments = "<role>")
	...
	@Subcommand(help = "Counts the members in this server.", name = "membercount", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	...
	@Subcommand(help = "Shows you the ID of this server and this channel.", name = "id", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	...
	@Subcommand(help = "Tells you what the default channel of this server is.", name = "defaultchannel", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	...
	@Subcommand(help = "Tells you some information about the given user.", name = "userinfo", parent = "com.impulsebot.commands.General.server", guildOnly = true, arguments = "<user>")
	...
	@Subcommand(help = "Some general information about this server.", name = "info", parent = "com.impulsebot.commands.General.server", guildOnly = true)
	...
	@Subcommand(help = "Shows you all of the channels in this server.", name = "channels", parent = "com.impulsebot.commands.General.server", guildOnly = true)
