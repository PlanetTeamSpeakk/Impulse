...
import com.impulsebot.utils.Zipper;
...
public class Trivia {
	...
	public TriviaResult start(MessageChannel channel, Guild guild) {
		...
		while (hasMoreQuestions()) {
			...
			else if (response.getContentDisplay().equalsIgnoreCase("stop trivia")) return new TriviaResult(appendees, false);
			while (!isCorrect(question, response.getContentDisplay())) {
				wrongAnswers += 1;
			...
			if (response != null && isCorrect(question, response.getContentDisplay())) {
	...
	public static class TriviaResult {
		...
		private TriviaResult(Map<User, Integer> appendees, boolean stoppedCuzNoResponse) {
			this.appendees = Main.sortByValue(appendees, true);
