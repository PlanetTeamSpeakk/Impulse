...
public class Downloader {
	...
	public static String getVideoLinkFromYoutubeVid(String url) throws IOException { // this code looks like a real mess, but it works perfectly!
		if (url.contains("youtu")) {
			Gson gson = new Gson();
			String downloadUrl = "";
			String data = Main.getHTML("http://api.w3hills.com/youtube/search?keyword=" + url + "&api_key=" + Main.apiKeys.get("w3hills"));
			if (!data.equals("[]")) {
				Map dataMap = gson.fromJson(data, Map.class);
				Map video = (Map) ((List) dataMap.get("videos")).get(0);
				String token = (String) video.get("token");
				String data1 = Main.getHTML("http://api.w3hills.com/youtube/get_video_info?video_id=" + url.split("v=")[1] + "&api_key=" + Main.apiKeys.get("w3hills") + "&token=" + token);
	...
	public static String getVideoLinkFromVimeoVid(String url) throws IOException {
		if (url.contains("vimeo.com/")) {
			Gson gson = new Gson();
			String data = Main.getHTML("https://player.vimeo.com/video/" + url.split("vimeo.com/")[1] + "/config");
