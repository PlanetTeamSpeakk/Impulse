...
public class CommandClientBuilder extends com.jagrosh.jdautilities.commandclient.CommandClientBuilder {
	...
	public CommandClientBuilder setStreaming(String name, String url)
	{
		if(name!=null && !name.isEmpty())
			if(Game.isValidStreamingUrl(url))
				setGame(Game.of(name, url));
			else
				setGame(Game.of(name));
		return this;
	}
